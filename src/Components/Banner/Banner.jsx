...
const Banner = ({coin, setCoin}) => {
  ...
  return (
    <div className='w-11/12 rounded-2xl mt-10 mx-auto grid justify-center p-20 bg-linear-to-tr from-blue-800 via-black to-red-700 opacity-90'>
      ...
      <button className={`bg-[#E7FE29] rounded-xl px-4 py-3 ${disable ? 'cursor-not-allowed' : `cursor-pointer`}`}onClick={handleDisable} disabled={disable}>Claim Free Credit</button>
      </div>
    </div>
  )
}
